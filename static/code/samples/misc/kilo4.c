...
void enable_raw_mode() {
  ...
  // toggle flags to enable raw mode
  raw.c_iflag &= ~(BRKINT | ICRNL | INPCK | ISTRIP | IXON);
  raw.c_oflag &= ~(OPOST);
  raw.c_cflag |= (CS8);
  raw.c_lflag &= ~(ECHO | ICANON | IEXTEN | ISIG);

  // set minimum number of bytes of input needed before read() can return to 0
  raw.c_cc[VMIN] = 0;

  // set maximum amount of time to wait before read() returns to 100ms
  raw.c_cc[VTIME] = 1;
...
// Later, we’ll expand this function to handle escape sequences, which involves reading multiple bytes that represent a single keypress, as is the case with the arrow keys.
int editor_read_key() {
  int nread;
  char c;
  ...
  // If we read an escape character, we immediately read two more bytes into the seq buffer. If either of these reads time out (after 0.1 seconds), then we assume the user just pressed the Escape key and return that.
  if (c == '\x1b') {
    char seq[3];
