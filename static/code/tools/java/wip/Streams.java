...
// a stream represents a sequence of elements and supports various operations that can be performed on those elements

List<String> myList = Arrays.asList("a1", "a2", "b1", "c2", "c1");

// stream operations either return a stream so we can chain multiple operations (intermediate) or return a non-stream result (terminal)
...
// many stream operations accept a functional interface specifying the exact behavior of the operation

// these passed function typically must not modify the underlying data source of the stream (not-interfering) and must not depend on any mutable state from the outer scope which might change during execution (stateless)
...
// terminal operations return a result of a certain type while  intermediate operations return the stream itself so you can chain multiple method calls in a row


// streams are created on a source such as a collection
...
// forEach accepts a consumer to executed for each element of the stream (terminal)
list
  .stream()
  .forEach(System.out::println);

// filter accepts a predicate to filter the elements of the stream (intermediate)
list
  .stream()
  .filter((s) -> s.startsWith("a"))
  .forEach(System.out::println);
...
// sorted sorts the stream in natural order (unless a customer Comparator is provided)
list
    .stream()
    .filter((s) -> s.startsWith("a"))
    .forEach(System.out::println);

// notice that the ordering of the underlying collection is unchanged
...
Reduce#
// reduce accepts a function performs a reduction on the elements of 
