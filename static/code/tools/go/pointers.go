...
A pointer is simply a variable that holds the location in memory where a value is
stored.
...
Go gives you the choice to
use pointers or values for both primitives and structs.
...
When the compiler sees that a pointer to
a local variable is returned, the local variable’s value is stored on the
heap. Escape analysis

Go performs pointer escape analysis. If the pointer escapes the local stack, which it does in this case, the object is allocated on the heap.
